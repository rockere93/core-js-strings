...
function isPalindrome(str) {
  const strClear = str.toLowerCase().replace(/[^a-z0-9]/g, '');
  const strRevers = strClear.split('').reverse().join('');
  for (let i = 0; i < strClear.length; i += 1) {
    if (strClear[i] !== strRevers[i]) {
      return false;
    }
  }
  return true;
